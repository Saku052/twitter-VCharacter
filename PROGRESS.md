...
- 投稿はRailway上の3サービス（`1755` / `2145` / `0815`）による1日3回運用

### 残課題
- `memo_mq.memo` カラムにNOT NULL制約をつけるか検討（現在は`Option<String>` + `unwrap_or_default()`で対応）
- 投稿失敗時も`eprintln!`で終了コード0のままなので、Railway上は`SUCCESS`に見えて検知できない（後述「2026-08 障害対応」参照）
- `fetch_latest_memo`は`used_at IS NULL`の最古1件を取るため、滞留が溜まると古い話題から消化される
...
- **新ペルソナでの実投稿による最終確認: 完了**。Railway上でのcron自動実行により、新ペルソナ（「個人でゲームを作っているVtuber」）でのツイートが実際に本番投稿されたことを確認済み
- **Railwayへのデプロイ: 完了**。twitter-VCharacter・data-collectorともに本番環境で稼働中
...
## schedule-broker（秘書AI / Phase A・B 完了）

他のAIエージェントから呼ばれる**空き時間判定・予定登録API**。twitter-VCharacter専用ではなく、全プロジェクト横断で使う独立コンポーネント（将来的にリポジトリ分離の想定）。要件は[scratch/SECRETARY_REQUIREMENTS.md](scratch/SECRETARY_REQUIREMENTS.md)、使い方は[schedule-broker/README.md](schedule-broker/README.md)、他エージェントへの接続手順は[schedule-broker/AGENT_ONBOARDING.md](schedule-broker/AGENT_ONBOARDING.md)を参照。

### 決定事項

- **読み取りはGoogle Calendar、書き込みはTickTick**: TickTick Open APIに「タスク一覧取得」が存在せず（GETはID既知の1件のみ）、空き判定＝「予定が無いことの確認」が原理的に成立しないため。両者は同期済み
- **自然言語の解釈はしない**: 「来週の22時前」のような表現は呼び出し元のエージェントが構造化してから叩く。本APIはLLMを持たないため、呼ばれる回数が増えても課金が伸びない
- **判定順序**: `too_short` → `blackout`（勤務・睡眠）→ `outside_window` → `busy_calendar` → `busy_pending`。前半3つは生活パターンのみで決まるため、カレンダーを引かずに応答できる（上流障害時も「勤務中です」は返せる）
- **生活パターンは`availability.toml`に外出し**: 2026-06〜08の実カレンダー27件から導出

### 実装済み

- Phase A（空き判定）: `POST /v1/availability/check`, `POST /v1/availability/search`
- Phase B（予定登録）: `POST /v1/reservations`, `DELETE /v1/reservations/{id}`
- Railwayにデプロイ済み（`schedule-broker-production.up.railway.app`）

### 既知の問題

- **TickTickのDELETEが200を返すのにタスクが消えない**（2026-08時点、実機確認済み）。取り消しは`complete`（完了扱い）を主経路とし、DELETEは併用に留めている
- TickTick→Google Calendarの同期遅延に備え、登録した予約は`reservations`テーブルにも記録して空き判定時にFreeBusyとマージする（二重予約の防止）

---

## 2026-08 障害対応（X APIクレジット枯渇 → 収集ロジックの不具合発覚）

Phase6の要件定義に入る前に現状把握をしたところ、**19日間ツイートが停止していた**ことが判明。その調査から派生して収集側の不具合も2件見つかり、順に対応した。

### ① X APIクレジット枯渇（19日間の投稿停止）

- **症状**: 2026-08-05 を最後に投稿が成功しておらず、cronは動いているのに毎回 `402 Payment Required (credits depleted)` で失敗していた
- **原因**: X APIが2026年に従量課金（クレジット制）へ移行。旧Basicプランは5/21に廃止告知、6/1以降の請求サイクル終了時に自動移行され、その後クレジットを使い切って残高が `-$0.38` のマイナスに落ちていた。**コード側の不具合ではない**
- **単価**（公式ドキュメント確認済み）: 投稿作成 `$0.015` / 回、**URLを含む投稿は `$0.200` / 回（13倍）**、投稿読み取り `$0.005`
- **コスト試算**: 1日3回 × $0.015 = 月 $1.35 / 年 $16.4。$5チャージで約3.4ヶ月（次の枯渇は2026年12月上旬見込み）
- **対応**: ユーザーが$5チャージ。8/25の3回（08:18 / 17:55 / 21:49）すべて投稿成功を確認して復旧
- **発見が19日遅れた理由**: 投稿失敗は`eprintln!`が出るだけで終了コードは0のため、Railway上は`SUCCESS`に見える。ログ保持も約7日しかなく、開始時刻はDBの`used_at`から逆算した

### ② Qiita収集が枯れていた（`per_page`の設計ミス）

- **症状**: data-collectorは毎日動いているのに、Qiitaからの取得が**30日中7日・各1件のみ**（取得率23.3%）。ログは連日「すでに処理済み」でスキップされていた
- **原因**: [data-collector/src/adapters/qiita.rs](data-collector/src/adapters/qiita.rs) の `per_page=3`。「1回のバッチで投入する上限を3件にする」意図だったが、実際には「**APIから見る記事を上位3件に限定する**」動作になっていた。並び順は新着降順で固定のため、上位3件が処理済みだと**下位の未処理記事に永久に到達できない**。Phase5で一括投入を防ぐため`100→3`に絞ったのが裏目に出た形
- **修正方針**: 上限を掛ける場所を1段後ろにずらす。「取得は広く・投入を絞る」
  - `qiita.rs`: `per_page` を `3 → 20`
  - `main.rs`: `QIITA_MAX_PER_RUN = 3` を追加し、投入成功数が上限に達したら`break`
  - あわせて`qiita_total`を「取得件数」から「**実際に処理を試みた件数**」に変更（処理済みスキップを分母から外す。従来のままだと`per_page`拡大で成功率が落ちたように見えるため）
- **検証**: 本番DBの処理済み43件と実際のQiita APIレスポンスでループ挙動をシミュレーションし、スキップを飛び越えて下位の未処理記事まで到達して3件投入されることを確認。`cargo check`・`cargo test`（3件）ともにpass
- **結果**: 8/24のバッチでQiitaから3件投入（従来は0〜1件）。コミット `e1b1991`

### ③ Agent由来メモに前置き・見出しが混入していた

- **症状**: `memo_mq`に「以下、最終メモ2個です。」「`**メモ1（…）**`」「両方とも50文字以内に収まっています。」といった**Agentの独り言がメモとして保存されていた**。中には「CLAUDE.mdのルールに従って感想形式のメモを2個作成します。」という、投稿されると中の人がAIだと露見しかねないものもあった
- **原因**: [agent-wrapper/main.py](agent-wrapper/main.py) が**Agentの最後の発言を改行で割って、各行をそのままメモとして採用**していた。TASK_PROMPTに「前置き・見出しを含めるな」とは書いてあるが、**プロンプトの遵守が唯一の品質保証**でコード側の検証が無かった
- **実測**: Agentは毎日2件出すはずが、35日中9日（約26%）が想定外の件数（1件が8日、4件が1日）。**4日に1回は何かしら失敗していた**
- **修正**: `_is_valid_memo()` を追加し、採用前にコード側で弾く。除外パターンはURL・箇条書き記号・`**`・「以下、」・「メモ1」・`CLAUDE.md`等への言及・自己申告表現・80字超。弾いた行は `メモとして不採用: [...]` としてログ出力する
- **検証**: 削除した破損13件は**全件弾き**、過去の正常なagentメモ64件は**誤爆ゼロ**。実際に起きた4つの失敗パターン（前置き+見出し / 参考リンク / 自己申告 / 正常）を再現し、壊れた行だけ落として本物のメモ2件を救えることを確認。コミット `08927c9`
- **未検証**: data-collectorは1日1回（03:35）のため、コミット時点では**まだ一度も実行されていない**。次回バッチでの動作確認が必要
- **この修正の限界**: DBに保存する直前のフィルタであり、**Agentが前置きを喋る癖自体は直していない**。失敗の仕方によっては採用が1件に減る日があり得る（0件にはしない設計）。ログの発生頻度を見て、TASK_PROMPT側を調整するか判断する

### ④ 滞留メモの棚卸し（24件削除）

- 投稿停止中に未使用メモが70件まで積み上がった。復旧すると`fetch_latest_memo`が最古から取るため、**約23日間ずっと3週間前の話題を投稿し続ける**状態だった
- 内容を精査した結果、問題は日付の古さより**破損と重複**にあると判断し、24件を`DELETE`（`used_at IS NULL`のガード付き、削除前にTSVバックアップ取得）
  - 破損13件: 上記③の前置き・見出し・参考リンク行・URL付き
  - 重複11件: 「生成AIへの不安が倍増」が6件、11月銀座のカンファレンスが3件など。**各グループ1件は残した**
- 残り46件。正常なメモは日付を問わず温存した（内容が古びておらず、供給が消費を下回っているため）
...
|---|---|---|
| Agent SDK | 2件/日 | 安定 |
| Qiita | 0〜3件/日 | ②で修正。在庫消化後は新着ペース依存で0〜1件/日 |
| YouTube | **0件/日** | 07-19以降ゼロ。プレイリストの補充が必要（コードでは解決しない） |
| **消費** | **3件/日** | 1日3回投稿 |

**恒常的にはAgent SDK主体の約2件/日で、消費3件/日をわずかに下回る**。滞留を消化しきると枯れる方向のため、YouTubeプレイリストの補充かQiita条件の緩和（`stocks:>1`を外すと直近30日で74件）がいずれ必要。

### 次にやること

- **③の効果確認**（次回03:35のバッチ後）: 破損メモが入っていないか / Agent Wrapperのログに`メモとして不採用`が出る頻度。**毎日出るならTASK_PROMPT自体の調整が必要**というサイン。Railwayのログ保持は約7日なので1週間以内に見ること
- **8/25のバッチで投入が0件だった件**の原因調査（Agent SDKの2件すら入っていない）
- **監視の仕組み**: 投稿失敗時に非ゼロ終了してRailway上でFAILEDにする等。次のクレジット枯渇（12月上旬見込み）でも同じ見落としが起きる
