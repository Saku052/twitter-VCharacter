...
- **コスト上限**: 1回のバッチ実行あたり、検索回数・AI呼び出し回数にハードリミットを設ける方針（具体的な回数は未定、次回詰める）

### 未決事項（次回詰める）

- コスト上限の具体的な数値（検索回数・AI呼び出し回数の上限）
- 重複排除の仕組み（ソースが可変なため、`processed_videos`のようなソース別専用テーブル方式が使えない。URLベースの汎用テーブル等、代替設計が必要）
- Agentの出力を既存の`MemoWriter`/`insert_memo`フローにどう接続するか（既存のportsパターンとの統合方法）
- Agentへの指示プロンプトの具体的な内容（「トレンドを調べて」だけでは曖昧すぎるため、対象範囲・除外基準等の具体化が必要）
- 実行頻度（YouTube/Qiitaと同じ1日1回にするか、コスト・信頼性の観点から別頻度にするか）
- YouTube/Qiitaとの実行順序・関係性（既存2ソースと同じ`main.rs`内で直列にするか、独立した仕組みにするか）
